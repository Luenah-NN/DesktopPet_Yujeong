...
class Pet(QtWidgets.QMainWindow):
    ...
    def mouseMoveEvent(self, ev):
        ...
        if self.dragging:
            ...
            # ★ 이미 드래그로 만든 climb 상태에서 다시 드래그하면 → 떼어내서 hang
            if self.is_climbing and self.climb_locked_from_drag:
                ...
                if self.mode not in ("dance","sleep","exercise"):
                    self.set_action("hang", force=True, suppress_bounce=True)
                # 이 프레임에서는 다시 벽에 안 붙도록 종료
                return

            # 드래그 중 양쪽 벽 → climb (지금 climb 중이 아닐 때만)
            if (self.mode not in ("dance","sleep","exercise")
    ...
    def mouseReleaseEvent(self, ev):
        ...
        if self.dragging:
            ...
            # 드래그로 만든 climb인데 여전히 벽에 붙은 상태이면 → 그대로
            if self.is_climbing and self.climb_locked_from_drag:
                self.manual_drop = False
                self.free_bounce = False
                self.vx = 0.0
            ...
            # 여기로 오면: 드래그 중이었지만 이제는 climb이 아님 → 던지기/낙하
    ...
    def update_loop(self):
        ...
        # ⭐ 떨어지는 중이면 여기서 끝 → follow/random 안 타게
        if self.manual_drop:
            return
